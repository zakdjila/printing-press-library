...
Exit codes & warnings:
  ...
  vapi-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  vapi-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  vapi-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  vapi-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  vapi-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "assistant":
		return db.UpsertAssistant(data)
	case "call":
		return db.UpsertCall(data)
	case "campaign":
		return db.UpsertCampaign(data)
	case "chat":
		return db.UpsertChat(data)
	case "eval":
		return db.UpsertEval(data)
	case "file":
		return db.UpsertFile(data)
	case "observability":
		return db.UpsertObservability(data)
	case "phone_number":
		return db.UpsertPhoneNumber(data)
	case "provider":
		return db.UpsertProvider(data)
	case "session":
		return db.UpsertSession(data)
	case "squad":
		return db.UpsertSquad(data)
	case "structured_output":
		return db.UpsertStructuredOutput(data)
	case "vapi_analytics":
		return db.UpsertVapiAnalytics(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"eval": "/eval",
		"eval-run": "/eval/run",
		"observability": "/observability/scorecard",
