...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".vapi-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VAPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VAPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vapi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.vapi-pp-cli/feedback.jsonl.
When ` + "`VAPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VAPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vapi-pp-cli feedback list
  vapi-pp-cli feedback list --limit 5
  vapi-pp-cli feedback list --json`,
