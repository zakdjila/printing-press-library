...
In local mode: searches locally synced data only.`,
  ...
  vapi-pp-cli search "error timeout"

  # Force local search only
  vapi-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  vapi-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  vapi-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'vapi-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "eval":
				results, err = db.SearchEval(query, limit)
			case "observability":
				results, err = db.SearchObservability(query, limit)
			case "structured_output":
				results, err = db.SearchStructuredOutput(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchEval(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search eval failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchObservability(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search observability failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchStructuredOutput(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search structured_output failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
