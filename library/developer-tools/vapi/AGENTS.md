# Vapi Printed CLI Agent Guide

This directory is a generated `vapi-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
vapi-pp-cli doctor --json
vapi-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
vapi-pp-cli which "<capability>" --json
vapi-pp-cli <command> --help
```
...
```bash
vapi-pp-cli <command> --help
vapi-pp-cli <command> --dry-run --agent
```
