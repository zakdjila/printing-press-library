...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  x-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  x-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account-activity", "activity", "activity-subscriptions", "articles", "chat", "compliance", "connections", "dm-events", "openapi-json", "tweets", "tweets-search-webhooks", "usage", "users", "users-personalized-trends", "users-reposts-of-me", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  x-pp-cli workflow status

  # Show status as JSON
  x-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("x-pp-cli")
			}
