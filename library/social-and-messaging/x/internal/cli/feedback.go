...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".x-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("X_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("X_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "x-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.x-pp-cli/feedback.jsonl.
When ` + "`X_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`X_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  x-pp-cli feedback list
  x-pp-cli feedback list --limit 5
  x-pp-cli feedback list --json`,
