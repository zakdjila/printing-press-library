...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "x-pp-cli")
...
// GraphQL on x.com while v2 is on api.x.com).
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	if strings.HasPrefix(path, "http://") || strings.HasPrefix(path, "https://") {
		// Only allow absolute URLs for the X surface this CLI is built for.
		// Refusing arbitrary hosts prevents accidental token exfiltration if a
		// caller path becomes user-controllable.
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// browser-session cookie auth captured at sniff time (Source B).
		// All other hosts (api.x.com etc.) use the OAuth 2.0 bearer (Source A).
